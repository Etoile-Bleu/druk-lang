...
public:
    ...
    [[nodiscard]] void* allocate(size_t size, size_t alignment) {
        size_t current_ptr = reinterpret_cast<size_t>(blocks_.back().get()) + current_offset_;
        ...
        if (current_offset_ + padding + size > block_size_) {
            ...
            allocate_block();
            current_offset_ = 0;
            // Recalculate padding for new block (always 0 if block is aligned, which malloc usually is)
...
private:
    void allocate_block() {
        // Using unique_ptr to manage memory. C++ new[] aligns to max_align_t usually.
