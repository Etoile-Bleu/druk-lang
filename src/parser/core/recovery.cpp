...
void Parser::error(lexer::Token token, std::string_view message)
{
    ...
    diagnostic.location = {token.line, token.column, token.offset, token.length};
    diagnostic.message  = std::string(message);

    errors_.report(diagnostic);
}

void Parser::warn(lexer::Token token, std::string_view message)
{
    util::Diagnostic diagnostic;
    diagnostic.severity = util::DiagnosticsSeverity::Warning;
    diagnostic.location = {token.line, token.column, token.offset, token.length};
    diagnostic.message  = std::string(message);
